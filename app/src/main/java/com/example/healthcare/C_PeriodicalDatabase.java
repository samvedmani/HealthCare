...
class C_PeriodicalDatabase {
    ...
    private class PeriodicalDataOpenHelper extends SQLiteOpenHelper {
         ...
         */
        final static String DATABASE_NAME = "c_main.db";
        /**
    ...
    public static class DayEntry {
        ...
        int type;
        final C_GregorianCalendarExt date;
        int dayofcycle;
        int intensity;
        String notes;
        ...
        DayEntry(int type, GregorianCalendar date, int dayofcycle, int intensity) {
            this.type = type;
            this.date = new C_GregorianCalendarExt();
     ...
     * @param context Application context
     */
    C_PeriodicalDatabase(Context context) {
    ...
    private boolean backupRestore(boolean backup) {
        ...
        if (ok) {
            ...
            try {
                C_FileUtils.copyFile(new FileInputStream(sourceFile),
                        new FileOutputStream(destFile));
                // Try to copy journal only if it exists
                if (sourceFileJournal.exists())
                    C_FileUtils.copyFile(new FileInputStream(sourceFileJournal),
